...
class MarkovHead(torch.nn.Module):
    ...
    def __init__(self, params, action_shape, log_freq=10000):
        ...
        self.use_inverse = params["use_inverse"]
        self.use_ratio = params["use_ratio"]
        self.use_smoothness = params["use_smoothness"]
    ...
    def compute_markov_loss(self, z0, a, z1):
        ...
        markov_loss = torch.tensor(0, device=device, dtype=torch.float32)
        l_inverse = torch.tensor(0, device=device, dtype=torch.float32)
        l_contr = torch.tensor(0, device=device, dtype=torch.float32)
        l_smoothness = torch.tensor(0, device=device, dtype=torch.float32)
        ...
        if self.use_inverse:
            ...
            markov_loss += self.inverse_coef * l_inverse
        ...
        if self.use_ratio:
            ...
            markov_loss += self.contrastive_coef * l_contr

        # Smoothness loss
        if self.use_smoothness:
            # TODO: Should implement option for different kinds of smoothness losses
            with torch.no_grad():
            ...
            markov_loss += self.smoothness_coef * l_smoothness
