...
def evaluate(env, agent, video, num_episodes, L, step, args):
    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        for i in range(num_episodes):
            ...
            wandb.log({f"eval/{prefix}episode_reward": episode_reward}, step=step)
            all_ep_rewards.append(episode_reward)

        duration = time.time()-start_time
        L.log('eval/' + prefix + 'eval_time', duration , step)
        wandb.log({f"eval/{prefix}eval_time": duration}, step=step)
        mean_ep_reward = np.mean(all_ep_rewards)
        best_ep_reward = np.max(all_ep_rewards)
        std_ep_reward = np.std(all_ep_rewards)
        L.log('eval/' + prefix + 'mean_episode_reward', mean_ep_reward, step)
        L.log('eval/' + prefix + 'best_episode_reward', best_ep_reward, step)
        wandb.log({f"eval/{prefix}mean_episode_reward": mean_ep_reward}, step=step)
        wandb.log({f"eval/{prefix}best_episode_reward": best_ep_reward}, step=step)
        ...
        if done:
            if step > 0:
                if step % args.log_interval == 0:
                    ...
                    wandb.log({"train/duration": duration}, step=step)
                    L.dump(step)
                start_time = time.time()
            if step % args.log_interval == 0:
