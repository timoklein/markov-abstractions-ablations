...
def parse_args():
    ...
    parser.add_argument('--replicate', default=True, action='store_true')
    ...
    parser.add_argument('--markov', default=True, action='store_true')
    parser.add_argument('--markov_pretrain_steps', default=100000, type=int,
    ...
    parser.add_argument('--markov_catchup_steps', default=4000, type=int,
        help='Number of agent updates to catch up on after pretraining is complete')
    parser.add_argument('--markov_inv_coef', default=1.0, type=float)
    parser.add_argument('--markov_contr_coef', default=1.0, type=float)
    parser.add_argument('--markov_smoothness_coef', default=10.0, type=float)
    ...
    parser.add_argument('--tag', default="markov-agent", type=str)

    # Ablations
    parser.add_argument("--use_inverse", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
        help="argument for enabling inverse model learning of the markov abstraction")
    parser.add_argument("--use_ratio", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
        help="argument for enabling the density ratio objective")
    parser.add_argument("--use_smoothness", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
        help="argument for enabling for enabling the smoothness loss")
...
def evaluate(env, agent, video, num_episodes, L, step, args, run):
    all_ep_rewards = []
    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        for i in range(num_episodes):
            ...
            run.log({f"eval/{prefix}episode_reward": episode_reward}, step=step)
            all_ep_rewards.append(episode_reward)

        duration = time.time()-start_time
        L.log('eval/' + prefix + 'eval_time', duration , step)
        run.log({f"eval/{prefix}eval_time": duration}, step=step)
        mean_ep_reward = np.mean(all_ep_rewards)
        best_ep_reward = np.max(all_ep_rewards)
        std_ep_reward = np.std(all_ep_rewards)
        L.log('eval/' + prefix + 'mean_episode_reward', mean_ep_reward, step)
        L.log('eval/' + prefix + 'best_episode_reward', best_ep_reward, step)
        run.log({f"eval/{prefix}mean_episode_reward": mean_ep_reward}, step=step)
        run.log({f"eval/{prefix}best_episode_reward": best_ep_reward}, step=step)
...
def main(args: argparse.Namespace):
    ...
    device = utils.set_cuda_configuration(args.gpu)
    ...
    run = wandb.init(
        project="Markov Ablations",
        name=f"{args.domain_name}_{args.task_name}",
        config=vars(args)
    )
    ...
    for step in tqdm(range(args.num_train_steps)):
        ...
        if step % args.eval_freq == 0:
            ...
            run.log({"eval/episode": episode}, step=step)
            eval_data = evaluate(env, agent, video, args.num_eval_episodes, L, step, args, run)
            if args.save_model:
        ...
        if done:
            if step > 0:
                if step % args.log_interval == 0:
                    ...
                    run.log({"train/duration": duration}, step=step)
                    L.dump(step)
                start_time = time.time()
            if step % args.log_interval == 0:
                L.log('train/episode_reward', episode_reward, step)
                run.log({"train/episode_reward": episode_reward}, step=step)
            ...
            if step % args.log_interval == 0:
                ...
                run.log({"train/episode": episode}, step=step)
        ...
        if step >= args.init_steps:
            if not did_pretrain and args.markov and args.markov_pretrain_steps > 0:
                ...
                for pretrain_step in tqdm(range(args.markov_pretrain_steps)):
                    ...
                    agent.update_markov_head(pretrain_obs, pretrain_action, pretrain_next_obs, L, pretrain_step, run)
                if args.markov_catchup_steps > 0:
                    ...
                    for catchup_step in tqdm(range(step-args.markov_catchup_steps, step)):
                        agent.update(replay_buffer, L, catchup_step, args.markov_pretrain_steps, run)
                did_pretrain = True

            num_updates = 1
            for _ in range(num_updates):
                agent.update(replay_buffer, L, step, args.markov_pretrain_steps, run)
...
def get_settings_combinations(config, ablation_config):
    """Return a list of different combinations of settings to run.
    Each element in the list has the form:
    ( ("setting1", value1), ("setting2", value2), ... )
    """
    
    # Set the values that should be ablated in the general config
...
if __name__ == '__main__':
    ...
    # Dirty: Set the ablation variables here
    ablation_config = {
        "seed": (13, 1337, 34),
        "use_inverse": (True, False),
        "use_ratio": (False, True),
        "use_smoothness": (False, True),
        "gpu": 1
    }
    ...
    combinations = get_settings_combinations(args, ablation_config)

    # Can use nested list comprehension as depth of combinations is not arbitrary
    settings_to_iterate = set([inner[0] for tup in combinations for inner in tup])
    print("#" * 70)
    print(f"ITERATING OVER COMBINATIONS OF: {settings_to_iterate}.")
    print("#" * 70)
    # Cast as list to know the number of runs
    for combination in tqdm(combinations, desc="Runs completed"):
        # Generate the run combinations
        run_args = argparse.Namespace(**deepcopy(args))
        for key_value_pair in combination:
            setattr(run_args, key_value_pair[0], key_value_pair[1])

        # Manually set Markov parameters for this run from the general args to ensure consistency
        run_args.markov_params["use_inverse"] = run_args.use_inverse
        run_args.markov_params["use_ratio"] = run_args.use_ratio
        run_args.markov_params["use_smoothness"] = run_args.use_smoothness

        # Continue to run but enable KeyboardInterrupt
        try:
            settings = {key: value for key, value in combination}
            print("-" * 70)
            print(f"CURRENT RUN SETTINGS: {settings}.")
            print("-" * 70)
            if not (run_args.use_inverse or run_args.use_ratio):
                print("WARNING: SMOOTHNESS ONLY RUNS WILL BE SKIPPED")
            else:
