...
def random_observation_fn(n_states, n_obs_per_block):
    all_state_splits = [random_transition_matrix(size=(1, n_obs_per_block)) for _ in range(n_states)]
...
class AbstractMDP(MDP):
    def __init__(self, base_mdp, phi, pi=None, p0=None, t=200):
        ...
        self.T = [self.compute_Tz(self.belief, T_a) for T_a in base_mdp.T]
        self.R = [self.compute_Rz(self.belief, Rx_a, Tx_a, Tz_a) for (Rx_a, Tx_a, Tz_a) in zip(base_mdp.R, base_mdp.T, self.T)]
        self.T = np.stack(self.T)
        self.R = np.stack(self.R)
    ...
    def compute_Rz(self, belief, Rx, Tx, Tz):
        return np.divide((belief @ (Rx * Tx) @ self.phi), Tz, out=np.zeros_like(Tz), where=(Tz != 0))
